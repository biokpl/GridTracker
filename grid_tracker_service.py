...
def delete_excel_files():
    for f in [FILE1, FILE2]:
        if not f.exists():
            continue
        try:
            # Önce send2trash ile geri dönüşüm kutusuna göndermeyi dene
            try:
                import send2trash
                send2trash.send2trash(str(f))
                log.info(f"Geri dönüşüm kutusuna taşındı: {f.name}")
            except ImportError:
                # send2trash yoksa kalıcı sil
                f.unlink()
...
def setup_task_scheduler():
    if sys.platform != 'win32':
        log.error("Sadece Windows'ta çalışır."); return
    script = Path(__file__).resolve()
    python = sys.executable
    cmd = (
        f'schtasks /create /tn "GridBotTracker" /tr '
        f'"{python} {script} --now" '
        f'/sc WEEKLY /d MON,TUE,WED,THU,FRI /st 18:35 /f'
    )
    try:
        subprocess.run(cmd, shell=True, check=True)
        log.info("✓ Görev zamanlayıcıya eklendi: Pazartesi-Cuma 18:35")
    except subprocess.CalledProcessError as e:
        log.error(f"Hata: {e} — Yönetici olarak çalıştırın.")
...
if __name__ == '__main__':
    ...
    elif args.now:   run_once(dry_run=True)   # Test: Excel dosyalarini silmez
    else:            run_service()
